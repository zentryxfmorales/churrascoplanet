<?php
/**
 * ChurrascoPlanet Core - Compatibility Layer
...
// Definir función helper SIEMPRE primero (antes de cualquier return)
// para evitar errores de "Call to undefined function"
if (!function_exists('churrascoplanet_get_option')) {
    function churrascoplanet_get_option($key, $default = '') {
        // Si la clase no está disponible aún, usar fallback
...
// Si la clase ya existe (cargada por el tema), no redeclararla
if (class_exists('ChurrascoPlanet_Compat')) {
    return;
}
...
class ChurrascoPlanet_Compat {
    ...
    private function __construct() {
        ...
        // Solo hacer preload después de que WordPress esté listo
        if (did_action('plugins_loaded')) {
            $this->preload_cache();
        } else {
            add_action('plugins_loaded', array($this, 'preload_cache'), 10);
        }
    }
    ...
    public function preload_cache() {
        ...
        // Verificar que la clase de configuración existe
        if (!class_exists('ChurrascoPlanet_Config_Model')) {
            return;
        }
    ...
    private function tables_exist() {
        ...
        if ($exists === null) {
            ...
            // Verificar que la función chp_table existe
    ...
    public function get_option($key, $default = '') {
        ...
        // Verificar que chp_config existe antes de usarla
    ...
    public function set_option($key, $value) {
        if (!$this->tables_exist() || !function_exists('chp_set_config') || !function_exists('chp_table')) {
            return false;
        }
...
// Usamos add_action para asegurar que se inicialice después de que todo esté cargado
add_action('plugins_loaded', function() {
    ChurrascoPlanet_Compat::get_instance();
}, 5);

// También inicializar en init para el frontend
...
// Nota: La función churrascoplanet_get_option() se define al INICIO de este archivo
