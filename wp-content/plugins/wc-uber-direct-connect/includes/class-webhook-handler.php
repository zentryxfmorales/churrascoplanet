...
/**
 * Clase WCUDC_Webhook_Handler
 *
 * Procesa los webhooks enviados por Uber Direct
 */
class WCUDC_Webhook_Handler {
            ...
            default:
                ...
                $order->add_order_note(
                    sprintf(
                        ...
                        __( 'Evento de Uber recibido: %s', 'wc-uber-direct-connect' ),
                        $event_type
                    )
                );
        }
    ...
    private function handle_pickup( WC_Order $order, array $data ): void {
        ...
        $courier_name = $data['courier']['name'] ?? __( 'Courier', 'wc-uber-direct-connect' );

        $order->add_order_note(
            sprintf(
                /* translators: %s: courier name */
                __( 'El pedido ha sido recogido por %s (Uber Direct).', 'wc-uber-direct-connect' ),
                $courier_name
            )
        );

        // Disparar acción para integraciones externas
        do_action( 'wcudc_delivery_picked_up', $order, $data );
    }
    ...
    private function handle_dropoff( WC_Order $order, array $data ): void {
        ...
        $order->update_status(
            'completed',
            __( 'Pedido entregado por Uber Direct.', 'wc-uber-direct-connect' )
        );

        // Disparar acción para integraciones externas
        do_action( 'wcudc_delivery_completed', $order, $data );
    }
    ...
    private function handle_courier_approaching( WC_Order $order, array $data ): void {
        ...
        $order->add_order_note(
            sprintf(
                ...
                __( 'El courier de Uber está en camino al restaurante. ETA: %s minutos.', 'wc-uber-direct-connect' ),
                $eta
            )
        );

        do_action( 'wcudc_courier_approaching', $order, $data );
    }
    ...
    private function handle_cancelled( WC_Order $order, array $data ): void {
        ...
        $reason = $data['cancellation_reason'] ?? __( 'No especificada', 'wc-uber-direct-connect' );

        $order->add_order_note(
            sprintf(
                /* translators: %s: cancellation reason */
                __( 'Delivery de Uber cancelado. Razón: %s', 'wc-uber-direct-connect' ),
                $reason
            )
        );

        do_action( 'wcudc_delivery_cancelled', $order, $data );
    }
}
