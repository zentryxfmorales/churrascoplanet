<?php
/**
 * ChurrascoPlanet Core - Migration Manager
...
class ChurrascoPlanet_Migration {
    ...
    private function __construct() {
        ...
        // NOTA: El submenú ahora se registra en class-admin-options.php
        ...
        add_action('wp_ajax_churrascoplanet_install_tables', array($this, 'ajax_install_tables'));
    }
    ...
    public function ajax_install_tables() {
        ...
        $all_exist = array_reduce($tables, fn($carry, $t) => $carry && $t['exists'], true);

        if ($all_exist) {
            wp_send_json_success(array(
                'message' => 'Tablas instaladas correctamente.',
                'tables'  => $tables,
            ));
        } else {
            wp_send_json_error(array(
                'message' => 'Algunas tablas no pudieron crearse.',
                'tables'  => $tables,
            ));
        }
    }
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                alert('✗ ' + (response.data ? response.data.message : 'Error al reinstalar tablas'));
                            }
                        },
                        error: function() {
                            alert('Error de conexión al reinstalar tablas');
                        },
                        complete: function() {
