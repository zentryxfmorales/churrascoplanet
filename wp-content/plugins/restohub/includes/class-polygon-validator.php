...
/**
 * Clase RestoHub_Polygon_Validator
 *
 * Proporciona funciones de geofencing para asignar tiendas según ubicación del cliente
 */
class RestoHub_Polygon_Validator {

    /**
     * Instancia única (Singleton)
     *
     * @var RestoHub_Polygon_Validator|null
     */
    private static ?RestoHub_Polygon_Validator $instance = null;
     ...
     *
     * @return RestoHub_Polygon_Validator
     */
    public static function instance(): RestoHub_Polygon_Validator {
     ...
     * las coordenadas del cliente. Si hay múltiples tiendas con zonas superpuestas,
     * retorna la más cercana al punto.
     *
    ...
    public function find_store_for_point( array $point ): ?array {
        ...
        // Obtener todas las tiendas que cubren el punto
        $matching_stores = $this->find_all_stores_for_point( $point );
        ...
        // Si solo hay una tienda, retornarla directamente
        if ( count( $matching_stores ) === 1 ) {
            $store = reset( $matching_stores );
        ...
        // Si hay múltiples tiendas (zonas superpuestas), elegir la más cercana
        $this->log( 'info', sprintf(
            'Punto cubierto por %d tiendas. Seleccionando la más cercana.',
            count( $matching_stores )
        ) );

        return $this->get_nearest_from_list( $point, $matching_stores );
    }

    /**
     * De una lista de tiendas, retorna la más cercana al punto
     *
     * @param array $point  Coordenadas del cliente.
     * @param array $stores Lista de tiendas a evaluar.
     * @return array|null   Tienda más cercana.
     */
    private function get_nearest_from_list( array $point, array $stores ): ?array {
        if ( empty( $stores ) ) {
            return null;
        }
        ...
        if ( $nearest_store ) {
            $this->log( 'info', sprintf(
                'Tienda más cercana seleccionada: "%s" a %.2f km',
                $nearest_store['name'],
                $shortest_distance
            ) );
        }
    ...
    public function validate_polygon( array $polygon ): array {
        ...
        if ( count( $polygon ) < 3 ) {
            $errors[] = __( 'El polígono debe tener al menos 3 puntos.', 'restohub' );
        }
        ...
        foreach ( $polygon as $index => $point ) {
            if ( ! isset( $point['lat'] ) || ! isset( $point['lng'] ) ) {
                $errors[] = sprintf(
                    ...
                    __( 'El punto %d no tiene coordenadas válidas.', 'restohub' ),
                    $index + 1
                );
                continue;
            }
            ...
            if ( $lat < -90 || $lat > 90 ) {
                $errors[] = sprintf(
                    /* translators: %d: point index */
                    __( 'El punto %d tiene latitud fuera de rango (-90 a 90).', 'restohub' ),
                    $index + 1
                );
            }
            ...
            if ( $lng < -180 || $lng > 180 ) {
                $errors[] = sprintf(
                    /* translators: %d: point index */
                    __( 'El punto %d tiene longitud fuera de rango (-180 a 180).', 'restohub' ),
                    $index + 1
                );
            }
        }
        ...
        if ( empty( $errors ) && $this->get_polygon_area( $polygon ) === 0.0 ) {
            $errors[] = __( 'Los puntos del polígono son colineales (no forman un área).', 'restohub' );
        }
    ...
    /**
     * Obtiene las tiendas activas desde el repositorio
     *
     * @return array
     */
    private function get_active_stores(): array {
        ...
        if ( function_exists( 'restohub_store_repository' ) ) {
            return restohub_store_repository()->get_active_with_polygons();
        }

        // Fallback a wp_options (para retrocompatibilidad durante migración)
 ...
 * @return RestoHub_Polygon_Validator
 */
function restohub_polygon_validator(): RestoHub_Polygon_Validator {
    return RestoHub_Polygon_Validator::instance();
}
